...
// Response helper — format konsisten
...
app.post('/api/v1/transfers', async (req, res) => {
    ...
    if (!validWallets.includes(w)) return resBad(res, `Wallet: ${validWallets.join(', ')}`);
...
// 404
app.use('/api/v1', (req, res) => res.status(404).json({ status: 404, success: false, message: 'Endpoint tidak ditemukan' }));
