...
async function hitOrderkuotaAPI(url, data) {
    ...
    } catch (error) { return { success: false, message: "Koneksi Error" }; }
}
...
app.get('/api/saldo', async (req, res) => {
    ...
    if (result.success && result.account) res.json({ success: true, balance: result.account.results.balance, qris_balance: result.account.results.qris_balance });
...
app.get('/api/qris', async (req, res) => {
    ...
    try {
        ...
        if (response.data.status === 'success') res.json({ success: true, qris_base64: response.data.qris_base64 });
        else res.json({ success: false, message: response.data.message });
    } catch (error) { res.json({ success: false, message: "Gagal generate QRIS" }); }
});

// 3. MUTASI
app.get('/api/mutasi', async (req, res) => {
    const result = await hitOrderkuotaAPI('https://app.orderkuota.com/api/v2/qris/mutasi/1349636', { 'requests[0]': 'account', 'requests[qris_history][page]': '1', 'requests[qris_history][dari_tanggal]': '', 'requests[qris_history][ke_tanggal]': '', 'requests[qris_history][keterangan]': '' });
    const transactions = result.qris_history?.results;
    if (result.success && transactions && transactions.length > 0) { const cleanList = transactions.map(t => ({ ...t, kredit_clean: cleanNumber(t.kredit), status: t.status })); res.json({ success: true, data: cleanList }); }
...
app.post('/api/order', async (req, res) => {
    ...
    if (wallet === 'OVO') voucherId = CONFIG.VOUCHER_OVO; if (wallet === 'GOPAY') voucherId = CONFIG.VOUCHER_GOPAY; if (wallet === 'SHOPEEPAY') voucherId = CONFIG.VOUCHER_SHOPEEPAY;
    try {
        ...
        if (result.success) res.json({ success: true, idTrx: result.results?.id || result.data?.id || "N/A" });
        else res.json({ success: false, message: result.message });
    } catch (error) { res.json({ success: false, message: "Gagal memproses order" }); }
});
...
app.post('/api/withdraw', async (req, res) => {
    ...
    try {
        ...
        if (result.success && result.qris_withdraw && result.qris_withdraw.success) res.json({ success: true, message: `Saldo Rp ${Number(amount).toLocaleString('id-ID')} berhasil dipindahkan ke saldo utama.` });
        else res.json({ success: false, message: result.message || "Gagal memproses transfer" });
    } catch (error) { res.json({ success: false, message: "Terjadi kesalahan server" }); }
});

// 6. TOPUP DANA BEBAS NOMINAL (BBSD)
app.post('/api/topup', async (req, res) => {
    const { phone, nominal } = req.body;
    if (!phone || !nominal) return res.json({ success: false, message: "Nomor dan nominal wajib" });
    try {
        const result = await hitOrderkuotaAPI('https://app.orderkuota.com/api/v2/order', { quantity: "1", id_plgn: nominal, kode_promo: "", pin: "", phone: phone, voucher_id: "BBSD", payment: "balance" });
        if (result.success) res.json({ success: true, idTrx: result.results?.id || result.data?.id || "N/A" });
        else res.json({ success: false, message: result.message });
    } catch (error) { res.json({ success: false, message: "Gagal memproses top up" }); }
});
