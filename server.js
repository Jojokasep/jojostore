...
const QRIS_RAW = "00020101021126670016COM.NOBUBANK.WWW01189360050300000879140214508257991305870303UMI51440014ID.CO.QRIS.WWW0215ID20232921284770303UMI5204541153033605802ID5920JOJO STORE OK13496366006CIAMIS62070703A0163045679";
...
// ============================================================
// QRIS: CRC16-CCITT & Custom Name Builder (FIXED WITH PADDING)
// ============================================================
...
function buildQrisWithName(name) {
    ...
    while (i < QRIS_RAW.length - 4) {
        ...
        tlvs.push({ tag, val, len });
        i += 4 + len;
    }
    
    // Cari dan ganti Tag 59 (Merchant Name)
    const idx = tlvs.findIndex(t => t.tag === '59');
    if (idx !== -1) {
        // Ambil panjang asli dari QRIS mentah (contoh: 20)
        const originalLength = tlvs[idx].len; 
        
        // Bersihkan nama, ubah ke uppercase, dan batasi max sesuai panjang asli
        let cleanName = String(name || CONFIG.DEFAULT_STORE_NAME).toUpperCase().substring(0, originalLength);
        
        // TAMBAHKAN PADDING (SPASI) DI BELAKANG NAMA
        // Tujuannya agar panjang string TETAP SAMA, mencegah Tag 62 (NPK) rusak/tergeser
        while (cleanName.length < originalLength) {
            cleanName += ' ';
        }
        
        tlvs[idx].val = cleanName;
    }
    
    // Rebuild tanpa CRC (Tag 63)
    let payload = '';
    for (const t of tlvs) {
        if (t.tag === '63') continue; // Skip CRC lama
...
// QRIS — sekarang support ?name=NAMA+TOKO (Dengan Padding Aman)
app.get('/api/qris', async (req, res) => {
    ...
    // Build QRIS dengan nama custom + padding aman
    ...
    try {
        ...
        if (r.data.status === 'success') {
            // Hapus spasi padding saat mengirimkan respons ke frontend agar tampil rapi
            const cleanDisplayName = storeName.toUpperCase().trim(); 
            res.json({ success: true, qris_base64: r.data.qris_base64, amount: Number(amount), store_name: cleanDisplayName });
        }
...
app.get('/', (req, res) => {
    res.json({
        status: 'ok', service: 'JoJo Store API', version: '2.2',
        endpoints: ['/api/saldo', '/api/qris?amount=&name=', '/api/mutasi', '/api/order', '/api/withdraw', '/api/webhook', '/api/sse']
    });
});

app.listen(PORT, () => console.log(`JoJo Store API v2.2 berjalan di port ${PORT}`));