...
// ============================================================
// SSE (Server-Sent Events) UNTUK REALTIME NOTIF
// ============================================================
...
// ============================================================
// AUTO-POLLING QRIS (MENGECEK PEMBAYARAN BARU)
// ============================================================
let isPolling = false;
...
// ============================================================
// API: QRIS (DIPERBARUI - DELEGASI KE FRONTEND)
// ============================================================
app.get('/api/qris', async (req, res) => {
    // Endpoint ini tidak lagi memproses QRIS karena sudah dihandle 100% oleh Frontend (Vercel)
    // Kode ini disimpan hanya sebagai fallback jika ada yang mencoba akses lewat Postman/curl
    res.json({ 
        success: false, 
        message: "Fitur QRIS Dinamis & Custom Name kini diproses langsung di Frontend untuk mencegah error parsing. Silakan gunakan tombol Generate di aplikasi web." 
    });
});

app.get('/api/mutasi', async (req, res) => {
    const r = await hitApi('https://app.orderkuota.com/api/v2/qris/mutasi/1349636', { 
        'requests[0]': 'account', 
        'requests[qris_history][page]': req.query.page || '1', 
        'requests[qris_history][dari_tanggal]': req.query.dari || '', 
        'requests[qris_history][ke_tanggal]': req.query.sampai || '', 
...
// ============================================================
// API: TRANSAKSI ORDER (KIRIM E-WALLET)
// ============================================================
...
app.get('/', (req, res) => {
    res.json({
        status: 'ok', service: 'JoJo Store API', version: '4.0-Clean',
        info: "QRIS Generator moved to Frontend",
        endpoints: ['/api/saldo', '/api/mutasi', '/api/order', '/api/withdraw', '/api/webhook', '/api/sse']
    });
});

app.listen(PORT, () => console.log(`JoJo Store API v4.0-Clean berjalan di port ${PORT}`));